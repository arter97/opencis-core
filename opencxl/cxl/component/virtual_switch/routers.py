...
class MmioRouter(CxlRouter):
    ...
    async def _process_host_to_target_packets(self):
        while True:
            ...
            vppb_downstream_connection = self._downstream_connections[target_port].vppb.get_upstream_connection()
            if vppb_downstream_connection is None:
                logger.error(self._create_message(f"vppb_downstream_connection for port {target_port} is None"))
                continue
